export default function FAQs() {
    return (
        <div className="flex flex-col items-center justify-center h-screen p-4 bg-gradient-to-r from-cyan-500 to-blue-500">
            <h1 className="text-6xl font-bold text-white">FAQs</h1>
            <ul className="mt-8 space-y-4 text-white">
                <li className="flex items-center">
                    <svg className="w-6 h-6 mr-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6-2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span>What is the best way to contact you?</span>
                </li>
                <li className="flex items-center">
                    <svg className="w-6 h-6 mr-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                    </svg>
                    <span>How do I reset my password?</span>
                </li>
            </ul>
        </div>
    );
}